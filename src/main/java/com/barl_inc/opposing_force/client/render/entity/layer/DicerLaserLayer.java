...
public class DicerLaserLayer extends RenderLayer<Dicer, DicerModel> {
    ...
    @Override
    public void render(PoseStack poseStack, MultiBufferSource buffer, int packedLight, Dicer dicer, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        if (dicer.isInvisible()) {
            return;
        }
